...
async function generatePosts() {
  ...
  for await (const file of Deno.readDir(POSTS_DIR)) {
    ...
    // console.log(markup);
    const newPostFileName =
      markup.meta.title.toLowerCase().replace(/ /g, '-') + '.html';
    const post = {
    ...
    let templateResult = await renderFile('./post', {
      blog: BLOG,
      title: post.title,
      date: post.date,
      author: post.author,
    ...
    Deno.writeTextFile(PUBLISH_DIR + newPostFileName, templateResult || '');
...
async function generateHomepage() {
  ...
  // Write the file to the PUBLISH_DIR
  await Deno.writeTextFile(PUBLISH_DIR + 'index.html', templateResult || '');
}
